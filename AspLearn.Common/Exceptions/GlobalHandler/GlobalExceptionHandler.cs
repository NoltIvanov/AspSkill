...
using AspLearn.Common.Exceptions.UserExceptions.Contracts;
...
namespace AspLearn.Common.Exceptions.GlobalHandler {
    ...
    public class GlobalExceptionHandler : IGlobalExceptionHandler {
        ...
        private async Task HandleServerException(HttpContext context, IExceptionHandlerFeature exceptionHandler, bool isDevelopment) {
            ...
            } else if (exceptionHandler.Error is IRouteConstraintException) {
                context.Response.StatusCode = (int)HttpStatusCode.Forbidden;
                await context.Response.WriteAsync((exceptionHandler.Error as IRouteConstraintException).GetUserMessageException).ConfigureAwait(false);
            }
